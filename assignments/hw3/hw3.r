...
# Your ggplot command:
p <- qplot(wr1500m$new_year, wr1500m$times_sec, geom="step")
p + geom_segment(aes(x=wr_1998, y=min(wr1500m$times_sec), xend=2015, yend=min(wr1500m$times_sec)))
...
# Your ggplot command
p + geom_vline(xintercept=wr_1944, col="green")
p + geom_vline(xintercept=wr_1998, col="green")
p + annotate("text", x=wr_1944 - 5, y=max(wr1500m$times_sec) - 15, col="blue", size=2, label=(wr1500m[which(wr1500m$new_year == wr_1944),])$athlete)
p + annotate("text", x=wr_1998 - 7, y=max(wr1500m$times_sec) - 15, col="blue", size=2, label=(wr1500m[which(wr1500m$new_year == wr_1998),])$athlete)
...
p <- qplot(wr1500m$new_year, wr1500m$times_sec, xlab="Year", ylab="Time (sec)", geom="step")
p <- p + geom_segment(aes(x=wr_1998, y=min(wr1500m$times_sec), xend=2015, yend=min(wr1500m$times_sec)))
p <- p + geom_vline(xintercept=wr_1944, col="green")
p <- p + geom_vline(xintercept=wr_1998, col="green")
p <- p + annotate("text", x=wr_1944 - 5, y=max(wr1500m$times_sec) - 15, col="blue", size=1.5, label=(wr1500m[which(wr1500m$new_year == wr_1944),])$athlete)
p <- p + annotate("text", x=wr_1998 - 7, y=max(wr1500m$times_sec) - 15, col="blue", size=1.5, label=(wr1500m[which(wr1500m$new_year == wr_1998),])$athlete)
p
...
# Your ggplot command
p <- qplot(x=log(SO2012Ctry$pop), y=log(SO2012Ctry$GDPPP))
p + geom_point(aes(size=SO2012Ctry$Total))
...
p <- qplot(x=log(SO2012Ctry$pop), y=log(SO2012Ctry$GDPPP), xlab="Population", ylab="GDP Per Person", main="Number of Medals Won by GDPPP by Population Size")
p <- p + geom_point(aes(size=SO2012Ctry$Total))
best <- order(SO2012Ctry$Total, decreasing=TRUE)[1:5]
p <- p + annotate(geom="text", x=log(SO2012Ctry$pop[best]), y=log(SO2012Ctry$GDPPP[best]), label=SO2012Ctry$Country[best], hjust=1, vjust=1)
p
######################################
# PLOT 3.
...
# Your ggplot commands:
map_info <- map_data("world",".", exact=FALSE)

qplot(long, lat, data = map_info, group=group) + geom_polygon(color="grey")
...
p <- qplot(long, lat, data = map_info, group=group) + geom_polygon(color="grey40")
p
...
n1989.rain <- sapply(sapply(sapply(day, floor), function(x) x==1989), sum)
