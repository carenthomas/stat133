...
set.seed(2718)
w <- sample(rep(1:13, 4), 13*4, replace=FALSE, prob=rep(1/52, 52))
...
t <- (infants[infants$marital == "Married" & infants$parity == 1, colnames(infants)])


# [2 pt]
# Calculate [mw], the average birthweight ($bwt) of all babies whose were full term, i.e. gestation equal or more than 259 days.
mw <- mean((infants[infants$gestation > 258, colnames(infants)])$bwt, na.rm=TRUE)
...
getFirst <- function(lst) {
  return(lst[1])
}

first.cache <- lapply(Cache500, getFirst)
...
mean.long.cache <- lapply(Cache500, val)
