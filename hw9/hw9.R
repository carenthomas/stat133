...
optionlist <- list(title="mpg v. disp for car models", vAxis="{title:'mpg'}",
                   hAxis="{title:'disp'}")
...
Motion <- gvisMotionChart(data=WorldDat, idvar="country", xvar="life.expectancy", yvar="fertility.rate", timevar="year", colorvar="region", sizevar="population", options=list(title="life expectancy v. fertility rate for countries", vAxis="{title:'fertility rate'}", hAxis="{title:'life expectancy'}"))
