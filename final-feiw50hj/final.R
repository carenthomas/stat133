...
name <- "Caren Thomas"
github.name <- "carenthomas"
email.address <- "carenthomas01@gmail.com"
...
z <- sample(x=logical, size=5000, replace=TRUE, prob=c(0.3, 0.7))
...
set.seed(31415)
v <- sample.int(n=6, size=100, replace=TRUE, prob=rep(1/6, 6))
...
set.seed(344)
m <- matrix(rnorm(n=10^2, mean=8), nrow=10)
...
RandomChick <- sample.int(n=578, size=200, replace=FALSE, prob=NULL)
cw4 <- ChickWeight[RandomChick, ]
...
# 
t2 <- table((subset(infants, ed=="College"))$smoke, (subset(infants, ed=="College"))$number)
...
#  
bdiff <- mean((subset(infants, age>34))$bwt) - mean((subset(infants, age<35))$bwt)
...
plot(x=mtcars$qsec, y=mtcars$hp, type="p") + abline(reg = lm(mtcars$qsec ~ mtcars$hp))
...
plot(x=mtcars$disp, y=mtcars$wt,  xlab = "Engine Displacement", ylab = "Weight", col = mtcars$am)
...
plot(y=WorldBank$life.expectancy, x=WorldBank$fertility.rate, type="p", ylab="Life Expectancy", xlab="Fertility Rate") + abline(v=2.1)
...
prop.rain <- lapply(X=rain, FUN=rainydays)
...
PermDiff <- function(cases, controls, k=5000){
  ...
  for (i in c(1:k)) {
    len <- length(cases) + length(controls)
    sample <- sample(x=c(cases, controls), size=len, replace=FALSE, prob=NULL)
    cases <- sample[1:length(cases)]
    controls <- sample[length(cases):len]
    n[[i]] <- mean(cases) - mean(controls)
  }
}
...
NumJackpot <- function(k, B) {
  n.jackpot <- vector(mode="numeric", length = B)
  for (i in c(1:B)) {
    win <- sort(sample.int(n=19, size=3, replace=FALSE, prob=NULL))
    winners <- 0
    for (j in c(1:k)) {
